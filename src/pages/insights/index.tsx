...
export default function Insights({ posts }: InsightsPageProps) {
  return (
    <Layout
      title="Insights - PrimeEdge by Involv"
      description="Latest insights and articles from PrimeEdge. Maximise your gaming performance with data-driven insights and EGM optimisation strategies."
    >
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold mb-4 text-white">PrimeEdge Insights</h1>
        ...
        {posts.length === 0 ? (
          <div className="text-center text-gray-400 py-12">
            ...
            <p>
              <Link href="http://localhost:3333" className="text-emerald-400 hover:underline">
                Open Sanity Studio
              </Link>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => {
              ...
              return (
                <article key={post._id} className="bg-gray-900/50 rounded-lg overflow-hidden border border-white/10 hover:border-emerald-500/30 transition-all duration-200">
                  {/* Main Image */}
                  {post.mainImage?.asset?.url ? (
                    ...
                    <div className="aspect-video w-full bg-gradient-to-br from-emerald-900/50 to-gray-800 flex items-center justify-center">
                  ...
                  <div className="p-6">
                    ...
                    {post.categories && post.categories.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {post.categories.slice(0, 2).map((category) => (
                          <span key={category._id} className="bg-emerald-600 px-2 py-1 rounded text-xs font-medium">
                            {category.title}
                          </span>
                        ))}
                    ...
                    <h2 className="text-xl font-semibold mb-3 line-clamp-2">
                      <Link href={`/insights/${post.slug.current}`} className="hover:text-emerald-400 transition-colors text-white">
                        {post.title}
                      </Link>
                    </h2>
        ...
        <div className="text-center mt-12">
          <Link href="/" className="text-emerald-400 hover:text-emerald-300 transition-colors flex items-center justify-center">
