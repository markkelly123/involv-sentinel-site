        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center">
              ...
              <img src="/logo-involv-primeedge-white2.svg" alt="PrimeEdge by Involv" className="h-6 w-auto" />
            </Link>
          </div>
          ...
          <div className="md:hidden">
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-md text-gray-300 hover:text-emerald-400 hover:bg-emerald-400/5 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-400 transition-colors duration-200"
            >
              <span className="sr-only">Open main menu</span>
              <i className={`${isOpen ? 'hidden' : 'block'} lni lni-menu text-xl`}></i>
              <i className={`${isOpen ? 'block' : 'hidden'} lni lni-close text-xl`}></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className={`md:hidden ${isOpen ? 'block' : 'hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-[#0f1115]/95 backdrop-blur-md border-b border-white/10">
          ...
          <div className="pt-4 pb-2">
            <Link
              href="/login"
              className="block w-full text-center bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-all duration-200"
              onClick={() => setIsOpen(false)}
            >
              Login
            </Link>
